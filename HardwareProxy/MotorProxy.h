 ...
 * Note that the speed of the motor is adjusted for the length of the rotary
 * arm to keep a constant speed at the end of the arm.
 */
struct MotorProxy
{
...
/*
 * Start up the hardware  but leave other settings of the hardware alone. 
 */
void MotorProxy_enable(MotorProxy* const me);
/*
 * MUST be called AFTER configure() function. Turn on the hardware to a known
 * default state.
 */
 ...
 */
void MotorProxy_writeMotorSpeed(MotorProxy* const me, const DirectionType
	direction, unsigned int speed);
